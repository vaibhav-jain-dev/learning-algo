...
(function() {
    ...
    const problem = {
        ...
        solutions: {
            python: `def streaming_window_dedup(list, k):
    """
    Streaming Window Dedup
...
# Test cases
print(streaming_window_dedup([1,2,3,4,5], None))  # Expected: [1,2,3,4,5]
`,
            go: `package main

import "fmt"

// StreamingWindowDedup solves the Streaming Window Dedup problem.
// Nodes arrive in a stream. Maintain a linked list where each value appears at most k times. When a new node arrives, either append it or reject it in O(1) time.
// Time: O(n), Space: O(1)
func StreamingWindowDedup(list []int, k int) []int {
	result := make([]int, 0)
...
func main() {
	fmt.Println(StreamingWindowDedup([]int{1, 2, 3, 4, 5}, nil)) // Expected: [1,2,3,4,5]
}
`
        },
        twists: [],
