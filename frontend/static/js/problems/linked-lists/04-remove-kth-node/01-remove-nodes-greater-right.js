...
func main() {
    ...
    fmt.Println(toArray(result)) // [13 8]
        ...
        twists: [
            { title: 'Remove Nodes With Smaller Value on Right', difficulty: 'Medium', description: 'Remove all nodes that have a strictly smaller value anywhere to their right side.', whyDifferent: 'Flips the comparison direction. Instead of a monotonically decreasing stack, you need a monotonically increasing stack, changing which nodes survive.', example: 'list=[5,2,13,3,8]: nodes with smaller value to right: 5 has 2,3 to right (smaller exists). Keep nodes where no smaller value exists to right. Result=[2,3,8]? No, 2 has nothing smaller, so keep [2,3,8].' },
            { title: 'Remove Nodes Greater Than K', difficulty: 'Easy', description: 'Remove all nodes whose value is strictly greater than a given threshold k, regardless of neighboring values.', whyDifferent: 'Removes the relative comparison aspect entirely. This is a simple filter operation, but handling head removal and consecutive removals requires careful pointer management.', example: 'list=[5,2,13,3,8], k=5: remove 13 and 8. Result=[5,2,3].' },
            { title: 'Keep Only Local Maxima', difficulty: 'Medium', description: 'Remove all nodes that are NOT local maxima. A node is a local maximum if it is greater than or equal to both its neighbors (head and tail are considered local maxima).', whyDifferent: 'Requires looking at both left and right neighbors simultaneously, not just one direction. This changes the traversal pattern since you need prev, curr, and next references.', example: 'list=[5,2,13,3,8]: local maxima are 5 (head), 13 (>2 and >3), 8 (tail). Result=[5,13,8].' },
            { title: 'Remove Nodes Where Right Sum Is Greater', difficulty: 'Hard', description: 'Remove a node if the sum of all nodes to its right is strictly greater than its value.', whyDifferent: 'Requires computing suffix sums first (or computing on the fly via reverse traversal), comparing each node against an aggregate rather than individual elements.', example: 'list=[5,2,13,3,8]: right sums: 5 has right sum 26, 2 has 24, 13 has 11, 3 has 8, 8 has 0. Remove nodes where right sum > value: 5<26, 2<24, 3<8. Keep [13,8].' },
            { title: 'Mark Instead of Remove', difficulty: 'Medium', description: 'Instead of removing nodes, mark the nodes that would be removed by setting their value to -1. Return the modified list without actually deleting any nodes.', whyDifferent: 'Changes from pointer manipulation to value mutation. The identification logic is the same, but the implementation avoids the complexity of node deletion and relinking.', example: 'list=[5,2,13,3,8]: nodes 5,2,3 have greater values to their right. Result=[-1,-1,13,-1,8].' }
        ],
        similar: [
