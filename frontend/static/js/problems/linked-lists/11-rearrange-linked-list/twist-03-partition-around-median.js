...
(function() {
    ...
    const problem = {
        ...
        solutions: {
            python: `def partition_around_median(list, k):
    """
    Partition Around Median
...
# Test cases
print(partition_around_median([1,2,3,4,5], None))  # Expected: [1,2,3,4,5]
`,
            go: `package main

import "fmt"

// PartitionAroundMedian solves the Partition Around Median problem.
// Rearrange the list around its median value (the middle value if sorted). First find the median, then partition around it.
// Time: O(n), Space: O(1)
func PartitionAroundMedian(list []int, k int) []int {
	result := make([]int, 0)
...
func main() {
	fmt.Println(PartitionAroundMedian([]int{1, 2, 3, 4, 5}, nil)) // Expected: [1,2,3,4,5]
}
`
        },
        twists: [],
