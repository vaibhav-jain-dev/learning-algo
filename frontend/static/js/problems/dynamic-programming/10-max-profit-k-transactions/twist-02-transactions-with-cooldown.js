    ...
    const problem = {
        ...
        examples: [
            // Basic test case
            {
                ...
                output: 2,
                explanation: 'For this input, there are 2 valid positions that satisfy the transactions with cooldown criteria.'
            },
            {
                input: {"prices":[3,2,5,7,1,3],"k":3},
                output: 3,
                explanation: 'For this input, there are 3 valid positions that satisfy the transactions with cooldown criteria.'
            },
            {
                input: {"prices":[1,2,3,4,5],"k":3},
                output: 1,
                explanation: 'For this input, there is 1 valid position that satisfy the transactions with cooldown criteria.'
            },
            // Edge case
            {
                input: {"prices":[5],"k":3},
        ...
        solutions: {
            python: `def transactions_with_cooldown(prices, k):
    """
    Transactions With Cooldown
...
# Test cases
print(transactions_with_cooldown([5,11,3,50,60,90], 3))  # Expected: 2
print(transactions_with_cooldown([3,2,5,7,1,3], 3))  # Expected: 3
print(transactions_with_cooldown([1,2,3,4,5], 3))  # Expected: 1
`,
            go: `package main
...
func TransactionsWithCooldown(prices []int, k int) int {
	result := 0
...
func main() {
	fmt.Println(TransactionsWithCooldown([]int{5, 11, 3, 50, 60, 90}, 3)) // Expected: 2
	fmt.Println(TransactionsWithCooldown([]int{3, 2, 5, 7, 1, 3}, 3)) // Expected: 3
	fmt.Println(TransactionsWithCooldown([]int{1, 2, 3, 4, 5}, 3)) // Expected: 1
}
`
        },
        twists: [],
