...
(function() {
    ...
    function renderProblemHTML(problem) {
        ...
        if (problem.twists && problem.twists.length > 0) {
            var category = problem.category || 'arrays';
            ...
            html += '<p style="color:#64748b;margin-bottom:1rem;font-size:0.85rem;">These twists force a different thought process. Each requires re-thinking the core approach. Click to open the full problem.</p>\n';
            html += '<div style="display:flex;flex-direction:column;gap:0.75rem;">\n';

            problem.twists.forEach((twist, idx) => {
                var twistDiff = diffColors[twist.difficulty] || { bg: '#f1f5f9', text: '#64748b' };
                var twistName = twist.name || twist.title || 'Twist ' + (idx + 1);
                // Navigable twist card (has id) - opens dedicated page
                if (twist.id) {
                    html += '<div onclick="window.openProblem(\'' + category + '\', \'' + twist.id + '\')" style="background:#fffbeb;border:1px solid #fde68a;border-radius:8px;padding:1rem 1.25rem;display:flex;align-items:center;justify-content:space-between;gap:1rem;cursor:pointer;transition:all 0.2s;" onmouseover="this.style.borderColor=\'#f59e0b\';this.style.background=\'#fef3c7\'" onmouseout="this.style.borderColor=\'#fde68a\';this.style.background=\'#fffbeb\'">';
                    html += '<div style="display:flex;align-items:center;gap:0.75rem;">';
                    html += '<span style="color:#b45309;font-size:0.9rem;font-weight:700;">' + (idx + 1) + '.</span>';
                    html += '<span style="color:#92400e;font-weight:600;">' + twistName + '</span>';
                    html += '</div>';
                    html += '<div style="display:flex;align-items:center;gap:0.75rem;">';
                    ...
                    // Inline twist (no dedicated page yet) - expandable
