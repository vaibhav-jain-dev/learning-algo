...
func main() {
    ...
    fmt.Println(MaxSortedSubarrays([]int{4, 3, 2, 1, 0}))  // 1
        ...
        twists: [
            { name: 'Max Chunks with Arbitrary Values', difficulty: 'Hard', description: 'The array values are arbitrary (not a permutation of 0 to n-1). Find the maximum chunks to sort independently.', whyDifferent: 'Cannot rely on index-value correspondence. Must use prefix max and suffix min comparison across arbitrary values.', example: 'array = [2, 1, 3, 4, 4]. Chunks: [2,1], [3], [4], [4]. Max = 4.' },
            { name: 'Min Chunks to Sort', difficulty: 'Medium', description: 'Find the minimum number of chunks. The answer is always 1 (sort the whole array), but what about with a constraint that each chunk has max size K?', whyDifferent: 'Size constraint forces more chunks. Must find boundaries respecting both sortability and size limit.', example: 'array = [1, 0, 2, 3, 4], K = 3. Chunk [1,0] size 2, then [2,3,4] size 3. Min = 2.' },
            { name: 'Chunks with Equal Sums', difficulty: 'Very Hard', description: 'Find the maximum number of chunks such that each chunk has the same sum after sorting.', whyDifferent: 'Dual constraint: chunks must be independently sortable AND have equal sums, requiring careful boundary selection.', example: 'array = [1, 0, 2, 3]. Chunks [1,0] sum=1, [2] sum=2, [3] sum=3. Unequal. Max equal-sum chunks = 1.' },
            { name: 'Verify Chunk Split', difficulty: 'Medium', description: 'Given a proposed list of chunk boundaries, verify if sorting each chunk independently results in a fully sorted array.', whyDifferent: 'Verification is easier than optimization: check that max of each chunk <= min of next chunk, plus each chunk is sortable.', example: 'array = [1, 0, 2, 3, 4], chunks = [[0,1], [2,4]]. Check: max([1,0])=1 <= min([2,3,4])=2. Valid!' },
            { name: 'Max Chunks for Descending Sort', difficulty: 'Hard', description: 'Find maximum chunks that can be independently sorted in descending order, and when concatenated, give a fully descending array.', whyDifferent: 'All comparisons reverse: prefix min and suffix max instead of prefix max and suffix min.', example: 'array = [4, 3, 2, 1, 0]. Each element is its own chunk for descending. Max = 5.' }
        ],
        similar: [
