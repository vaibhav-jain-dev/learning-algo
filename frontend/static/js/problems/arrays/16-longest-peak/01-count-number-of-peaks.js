...
func main() {
    ...
    fmt.Println(CountNumberOfPeaks([]int{5, 4, 3, 4, 5}))  // 0
        ...
        twists: [
            { title: 'Count Peaks and Valleys', difficulty: 'Medium', description: 'Count both peaks (element greater than both neighbors) and valleys (element less than both neighbors). Return both counts.', whyDifferent: 'Must check both conditions simultaneously and handle edge cases where a peak and valley are adjacent.', example: 'array = [1, 3, 2, 4, 1]. Peaks: 3 and 4 (count 2). Valleys: 2 and 1 (count 1, but 1 is edge). Return {peaks: 2, valleys: 1}.' },
            { title: 'Count Prominent Peaks', difficulty: 'Hard', description: 'A prominent peak has a prominence of at least K: the minimum drop on either side before reaching a higher peak. Count such peaks.', whyDifferent: 'Prominence requires looking beyond immediate neighbors to find how far each peak stands above surrounding terrain.', example: 'array = [1, 5, 3, 8, 2], K = 3. Peak 5 has prominence 2 (drops to 3). Peak 8 has prominence 6. Only 8 counts for K=3.' },
            { title: 'Peaks in 2D Grid', difficulty: 'Hard', description: 'Given a 2D matrix of heights, count cells that are strictly greater than all 4 adjacent neighbors (up, down, left, right).', whyDifferent: 'Extends from 1D to 2D, requiring 4-neighbor comparisons and boundary handling for edge and corner cells.', example: 'matrix = [[1,4,3],[6,5,2],[7,3,1]]. Peak at (1,0) value 6: greater than 1, 5, 7? No (7>6). Check all cells.' },
            { title: 'Minimum Distance Between Peaks', difficulty: 'Medium', description: 'Find the minimum index distance between any two peaks in the array.', whyDifferent: 'After finding all peak indices, compute the minimum gap between consecutive peaks, a post-processing step.', example: 'array = [1, 3, 2, 4, 1, 5, 2]. Peaks at indices 1, 3, 5. Min distance = 2.' },
            { title: 'Count Peaks in Sliding Window', difficulty: 'Hard', description: 'For each window of size W sliding across the array, count how many peaks exist within that window.', whyDifferent: 'Peaks must be recounted as the window slides, requiring efficient addition/removal of peak status at window boundaries.', example: 'array = [1, 3, 2, 4, 1, 5, 2], W = 3. Windows: [1,3,2]->1 peak, [3,2,4]->0, [2,4,1]->1, etc.' }
        ],
        similar: [
