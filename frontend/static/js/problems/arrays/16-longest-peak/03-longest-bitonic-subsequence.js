...
func main() {
    ...
    fmt.Println(LongestBitonicSubsequence([]int{1, 2, 3, 4, 5}))  // 5
        ...
        twists: [
            { title: 'Longest Bitonic Subarray', difficulty: 'Medium', description: 'Find the longest contiguous subarray (not subsequence) that is bitonic: strictly increases then strictly decreases.', whyDifferent: 'Contiguous constraint makes it simpler in some ways but requires linear scan tracking current bitonic run length.', example: 'array = [1, 11, 2, 10, 4, 5, 2, 1]. Longest bitonic subarray: [2, 10, 4] or [4, 5, 2, 1] length 4.' },
            { title: 'Count Bitonic Subsequences', difficulty: 'Very Hard', description: 'Count the total number of distinct bitonic subsequences of length at least 3.', whyDifferent: 'Counting instead of finding the longest requires DP counting at each position, a multiplicative rather than max operation.', example: 'array = [1, 3, 2]. Only one bitonic subsequence: [1, 3, 2]. Count = 1.' },
            ...
            { title: 'Bitonic Sort Verification', difficulty: 'Medium', description: 'Given an array, determine if it is a bitonic sequence (increases then decreases) and find the peak index.', whyDifferent: 'Decision problem plus peak-finding. Can be done with binary search on the direction change point.', example: 'array = [1, 3, 5, 4, 2]. Bitonic with peak at index 2 (value 5). Return true and index 2.' },
            { title: 'Minimum Additions for Bitonic', difficulty: 'Very Hard', description: 'Find the minimum number of elements to insert into the array to make the entire array a bitonic sequence.', whyDifferent: 'The complement of longest bitonic subsequence: n - LBS gives removals, but insertions to make the full array bitonic is different.', example: 'array = [1, 5, 2, 4, 3]. Insert elements to make fully bitonic like [1, 2, 4, 5, 4, 3, 2].' }
        ],
        similar: [
